...
window.lastlink="https://turbowarp.org/784241376/";
...
function generateUri(code) {
    ...
    function shorten() {
       /* alert("DO NOT POST SHORTENED URLS ON SCRATCH AS IT CAN GET YOU MUTED.")
        const textarea = document.getElementById("shorten_output");
        const tus = "https://tinyurl.com/api-create.php?url=",igs="https://is.gd/create.php?format=simple&url=",vgs="https://v.gd/create.php?format=simple&url=";
        var is_gd,v_gd,tinyurl;
        $.get(tus, window.lastlink, function(data) {
            alert(data.text);
            tinyurl=data.text;
        });
        $.get(vgs, window.lastlink, function(data) {
            alert(data.text);
            v_gd=data.text;
        });
        $.get(igs, window.lastlink, function(data) {
            alert(data.text);
            is_gd=data.text;
        });
        textarea.value = `${is_gd}
        ${v_gd}
        ${tinyurl}`;*/alert("Sorry this is not done :(");
    }